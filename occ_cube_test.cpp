...
int main( int argc, char** argv ) {
  ...
  //create a brick 
  RefEntity* brick = gmt->brick( 10, 10, 10 );

  //DLIList<RefEntity*> model;
  //model.append_unique(brick);

  //  status = gti->export_solid_moidel( model, "cube_mem.stp", "STEP", 1 );
  //if (status != CUBIT_SUCCESS) {
  // PRINT_ERROR("Problems writing file");
  //  return 1;
  // }
  ...
  for(unsigned int i = 0 ; i < my_surfs.size(); i++)
    {
      ...
      this_surf->get_graphics(data, 5, 1e-03);
